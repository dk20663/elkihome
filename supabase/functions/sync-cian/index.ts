...
function parseICS(icsText: string): Array<{ uid: string; dtstart: string; dtend: string; summary: string }> {
  ...
  for (let i = 1; i < blocks.length; i++) {
    ...
    const summary = block.match(/SUMMARY:(.+)/)?.[1]?.trim() || "Циан";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const house of houses || []) {
      const feedUrl = (house.cian_ical_url || "").trim();
      if (!feedUrl) continue;
      ...
      const feedUids = new Set(events.map((ev) => `cian_${house.name}_${ev.uid}`));
      ...
      for (const ev of events) {
        ...
        const extUid = `cian_${house.name}_${ev.uid}`;
    ...
    console.error("sync-cian error:", err);
